...
func TestMapRemoveByFunc(t *testing.T) {
	type args[K comparable, V any] struct {
		m map[K]V
		f func(K, V) bool
	}
	...
	tests := []testCase[int, string]{
		{
			...
			want: map[int]string{1: "1", 3: "3"},
		},
		{
			name: "2",
			args: args[int, string]{
				m: map[int]string{1: "1", 2: "2", 3: "3"},
				f: func(k int, v string) bool { return v+"1" == "31" },
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := MapDeleteByFunc(tt.args.m, tt.args.f); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("MapDeleteByFunc() = %v, want %v", got, tt.want)
			}
		})
	}
}
